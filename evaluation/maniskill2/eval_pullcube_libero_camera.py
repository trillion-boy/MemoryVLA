"""
ManiSkill2 PullCube evaluation with LIBERO camera configuration.

Tests MemoryVLA model on PullCube-v1 environment using LIBERO-style camera settings.
Tracks terminated vs truncated episode endings for detailed analysis.

Usage (in Colab):
    # After loading the model as 'vla'
    from evaluation.maniskill2.eval_pullcube_libero_camera import run_evaluation
...
DEFAULT_CONFIG = {
    "save_dir": "/content/eval_results_pullcube_libero",
    "num_trials": 8,
    "max_steps": 100,
    "unnorm_key": "libero_object_no_noops",
    "task_instruction": "Grasp the cube on the table and pull it backward toward the red and white circular target behind it. Move the cube toward the robot base until it reaches the target zone.",
    "cfg_scale": 1.5,
    "use_ddim": True,
    "num_ddim_steps": 10,
}
...
def run_evaluation(
    ...
    num_trials: int = 8,
    max_steps: int = 100,
    task_instruction: str = "Grasp the cube on the table and pull it backward toward the red and white circular target behind it. Move the cube toward the robot base until it reaches the target zone.",
    unnorm_key: str = "libero_object_no_noops",
    save_dir: str = "/content/eval_results_pullcube_libero",
    cfg_scale: float = 1.5,
    use_ddim: bool = True,
    num_ddim_steps: int = 10,
    ...
    # Create environment
    if verbose:
        print("Creating PullCube environment with LIBERO camera...")
    ...
    if verbose:
        print(f"\n{'='*60}")
        print(f"LIBERO Camera Evaluation: PullCube-v1")
        print(f"{'='*60}")
    ...
    if verbose:
        print(f"\n{'='*60}")
        print(f"Evaluation Summary - PullCube (LIBERO Camera)")
        print(f"{'='*60}")
    ...
    with open(result_file, "w") as f:
        f.write(f"PullCube Evaluation Results (LIBERO Camera) - {timestamp}\n")
        f.write(f"{'='*60}\n\n")
...
if __name__ == "__main__":
    ...
    print("\nUsage:")
    print("  from evaluation.maniskill2.eval_pullcube_libero_camera import run_evaluation")
