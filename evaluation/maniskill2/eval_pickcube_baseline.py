    ...
    for trial in range(num_trials):
        ...
        for step in range(max_steps):
            ...
            if depth_correction and depth_model is not None:
                ...
                # Detect cube from DA V2 depth (use cache if available)
                if cached_cube_pos is None:
                    ...
                    if is_first_attempt:
                        # Side-by-side: RGB | Depth with markers
                        ...
                        # Blue cross at center on both
                        cv2.drawMarker(rgb_annotated, (c, c), (0, 0, 255), cv2.MARKER_CROSS, 15, 1)
                        cv2.drawMarker(depth_rgb, (c, c), (0, 0, 255), cv2.MARKER_CROSS, 15, 1)

                        if cached_cube_pos is not None:
                            cx_i, cy_i = int(cached_cube_pos[0]), int(cached_cube_pos[1])
                            # Red cross on detected position
                            cv2.drawMarker(rgb_annotated, (cx_i, cy_i), (255, 0, 0), cv2.MARKER_CROSS, 20, 2)
                            cv2.drawMarker(depth_rgb, (cx_i, cy_i), (255, 0, 0), cv2.MARKER_CROSS, 20, 2)
