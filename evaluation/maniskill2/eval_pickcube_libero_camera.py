"""
ManiSkill2 PickCube evaluation with LIBERO camera configuration.

Tests MemoryVLA model on PickCube-v1 environment using LIBERO-style camera settings.
Tracks terminated vs truncated episode endings for detailed analysis.

Usage (in Colab):
    # After loading the model as 'vla'
    %run evaluation/maniskill2/eval_pickcube_libero_camera.py

    # Or import and run:
    from evaluation.maniskill2.eval_pickcube_libero_camera import run_evaluation
    results = run_evaluation(vla, num_trials=10)
"""
...
DEFAULT_CONFIG = {
    "save_dir": "/content/eval_results_libero_cam",
    "num_trials": 10,
    "max_steps": 100,
    "unnorm_key": "libero_object_no_noops",
    "task_instruction": "Grasp the small red cube on the table with the gripper, lift it up, and move it to the green target sphere hovering in the air.",
    "cfg_scale": 1.5,
    "use_ddim": True,
    "num_ddim_steps": 10,
}
...
def get_rgb_from_obs(obs: dict) -> np.ndarray:
    ...
    Handles both ManiSkill2 (mani_skill) and older observation formats.
...
def run_evaluation(
    ...
    num_trials: int = 10,
    max_steps: int = 100,
    task_instruction: str = "Grasp the small red cube on the table with the gripper, lift it up, and move it to the green target sphere hovering in the air.",
    unnorm_key: str = "libero_object_no_noops",
    save_dir: str = "/content/eval_results_libero_cam",
    cfg_scale: float = 1.5,
    use_ddim: bool = True,
    num_ddim_steps: int = 10,
    ...
    if verbose:
        print(f"\n{'='*60}")
        print(f"LIBERO Camera Evaluation: PickCube-v1")
        print(f"{'='*60}")
    ...
    for trial in range(num_trials):
        ...
        if save_videos:
            ...
            video_path = os.path.join(
                save_dir,
                f"trial_{trial:02d}_{status_str}.mp4"
            )
...
if __name__ == "__main__":
    ...
    print("\nUsage:")
    print("  from evaluation.maniskill2.eval_pickcube_libero_camera import run_evaluation")
    print("  results = run_evaluation(vla, num_trials=10)")
