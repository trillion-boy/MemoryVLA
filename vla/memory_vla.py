...
class MemoryVLA(nn.Module):
    ...
    def predict_action_with_spatial_control(
        self,
        image: Image,
        instruction: str,
        target_grid: torch.Tensor,
        ...
        **kwargs: str,
        ...
        """
        Action prediction with training-free spatial grounding (Method A).

        Instead of ControlDiT (which fails because per_attn is dead),
        this replaces per_attn with manual spatial attention using SAM masks.

        No optimization loop, no gradients - just direct spatial feature injection.
        Much faster than ControlDiT and actually effective.

        Args:
            image: PIL Image
            instruction: Task instruction string
            target_grid: [N_patches] or [1, N_patches] binary tensor (from SAM mask)
            spatial_scale: Magnitude of spatial attention output (0.1-1.0 recommended).
                          Start small; the model was trained with per_attn output ≈ 0.
            (other args same as predict_action)
        ...
        # === Phase 1: Extract cog_tokens and per_tokens (no grad needed) ===
        with torch.inference_mode():
            with torch.autocast("cuda", dtype=autocast_dtype, enabled=(autocast_dtype == torch.bfloat16)):
                output = super(PrismaticVLM, self.vlm).generate(
                    input_ids=input_ids,
        ...
        # === Phase 2: Spatial control (training-free, no optimization) ===
        controller = SpatialGatingControl(self.action_model.net, scale=spatial_scale)
        ...
        with torch.inference_mode():
            ...
            if using_cfg:
                ...
                uncondition = uncondition.expand(B, *uncondition.shape[1:])
                z = torch.cat([cog_tokens, uncondition], 0)
                model_kwargs = dict(z=z, cfg_scale=cfg_scale)
                sample_fn = self.action_model.net.forward_with_cfg
            ...
            # Diffusion sampling with spatial attention override active
