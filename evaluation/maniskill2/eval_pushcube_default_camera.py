"""
ManiSkill2 PushCube evaluation with default ManiSkill camera.

Tests MemoryVLA model on PushCube-v1 environment using ManiSkill's default camera settings.
Tracks terminated vs truncated episode endings for detailed analysis.

Usage (in Colab):
    # After loading the model as 'vla'
    from evaluation.maniskill2.eval_pushcube_default_camera import run_evaluation
...
DEFAULT_CONFIG = {
    "save_dir": "/content/eval_results_pushcube_default",
    "num_trials": 8,
    "max_steps": 100,
    "unnorm_key": "libero_object_no_noops",
    "task_instruction": "Push the blue cube forward along the table toward the red and white circular target. Approach the cube from behind and slide it forward until it reaches the target zone.",
    "cfg_scale": 1.5,
    "use_ddim": True,
    "num_ddim_steps": 10,
}
...
def run_evaluation(
    ...
    num_trials: int = 8,
    max_steps: int = 100,
    task_instruction: str = "Push the blue cube forward along the table toward the red and white circular target. Approach the cube from behind and slide it forward until it reaches the target zone.",
    unnorm_key: str = "libero_object_no_noops",
    save_dir: str = "/content/eval_results_pushcube_default",
    cfg_scale: float = 1.5,
    use_ddim: bool = True,
    num_ddim_steps: int = 10,
    ...
    if verbose:
        print("Creating PushCube environment with default ManiSkill camera...")
    ...
    if verbose:
        print(f"\n{'='*60}")
        print(f"Default Camera Evaluation: PushCube-v1")
        print(f"{'='*60}")
    ...
    if verbose:
        print(f"\n{'='*60}")
        print(f"Evaluation Summary - PushCube (Default Camera)")
        print(f"{'='*60}")
    ...
    with open(result_file, "w") as f:
        f.write(f"PushCube Evaluation Results (Default Camera) - {timestamp}\n")
        f.write(f"{'='*60}\n\n")
...
if __name__ == "__main__":
    ...
    print("\nUsage:")
    print("  from evaluation.maniskill2.eval_pushcube_default_camera import run_evaluation")
